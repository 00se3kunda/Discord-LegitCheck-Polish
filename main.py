...
class BoostPage:
    def __init__(self, nitro_type, authorname, authoravatar, authortext, receiveravatar, receivername, receivertext):
        ...
        self.sender_message_datetime = self.sender_message_datetime.strftime('Dziś o %H:%M')
        ...
        self.receiver_message_datetime = self.receiver_message_datetime.strftime('Dziś o %H:%M')
...
@client.event
async def on_ready():
    await client.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="B00ST M4RK3T"))
...
async def on_message(message):
    ...
    if message.content.startswith(client.user.mention):
        await message.channel.send(f"{message.author.mention}, wykonaj komende /proof, aby uzyskać swój LegitCheck!")

class NitroProofCustom(discord.ui.Modal, title='Fake Nitro Proof System'):
    nitrotype = discord.ui.TextInput(
        label='Wybierz typ nitra:',
        style=discord.TextStyle.short,
        placeholder='Wybierz: classic, boost czy promo',
        required=True,
        max_length=7,
    )

    authortext = discord.ui.TextInput(
        label='Tekst wysyłany przez ciebie:',
        placeholder='Przykład: Gratulacje! Oto twój kod:',
    ...
    receivername = discord.ui.TextInput(
        ...
        style=discord.TextStyle.short,
        placeholder='Przykład: 00se3kunda',
        required=True,
        max_length=32,
    )

    receiveravatar = discord.ui.TextInput(
        label='Link do awatara odbiorcy:',
        style=discord.TextStyle.short,
        placeholder='Przykład: https://image.com/XXXXXXX.png',
        required=False,
    )

    receivertext = discord.ui.TextInput(
        label='Tekst wysłany przez odbiorcę:',
        style=discord.TextStyle.paragraph,
        placeholder='Przykład: OMG, dzięki!',
        required=True,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(f"Dowód wygenerowany! Sprawdź swoje DM.", ephemeral=True)
        await interaction.user.send(file=discord.File('proof.png'))
        

    async def on_error(self, interaction: discord.Interaction, error: Exception) -> None:
        await interaction.response.send_message(f'Ups! Coś poszło nie tak. Proszę spróbuj ponownie.', ephemeral=True)
...
class NitroProofId(discord.ui.Modal, title='Fake Nitro Proof System'):
    ...
    receiverid = discord.ui.TextInput(
        label='Identyfikator odbiorcy:',
        style=discord.TextStyle.short,
        placeholder='Przykład: 464457105521508354',
        required=True,
        max_length=18,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send(f'Ups! {e}', ephemeral=True)
            return
...
@client.tree.command(description='Wygeneruj swój LegitCheck.')
@app_commands.describe(receiverinfo='Znajdź nazwę/awatar konta z identyfikatorem lub dostosuj go')
@app_commands.choices(receiverinfo=[app_commands.Choice(name='Identyfikator odbiorcy', value='id'),app_commands.Choice(name='Niestandardowa osoba', value='custom')])
